...
3. Confirm generation mode copy is honest and useful:
   `Prototype generation mode` when credentials are absent, and grounded real generation when `GENERATION_MODE=real`, `MCP_SERVER_URL`, `BEDROCK_MODEL_ID`, `AWS_REGION`, and either AWS credentials or `AWS_BEARER_TOKEN_BEDROCK` are configured.
...
6. If a row has weak evidence, confirm the draft degrades safely with low confidence and consultant-review wording instead of inventing MES behavior.
7. If the run fails because the MCP or Bedrock path is inaccessible, record the exact blocker in a dated discovery note instead of marking Phase 1 complete.

Still future-facing:

- Exact click-by-click MES documentation lookup depends on documentation quality and may still require prompt tuning
