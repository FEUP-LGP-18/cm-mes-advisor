...
There is no app database for Phase 1 project state. Prototype project data is stored in the browser with `localStorage`. Server routes are used for generation and availability boundaries.
...
Never move AWS, Bedrock, MCP, MES, or partner secrets into client components.
