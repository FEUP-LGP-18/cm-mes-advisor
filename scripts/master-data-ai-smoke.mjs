...
const smokeUrl = new URL("/api/master-data/ai-smoke", targetBaseUrl);
const headers = {
...
if (!response.ok || !body?.ok) {
  fail(
    "Master Data AI smoke route did not report a successful real generation run.",
  );
}

assertSmokeResult(body, { expectProvider });
  ...
  return {
    ...
    generation: generation
      ? {
          ...
          totalObjects: generation.totalObjects,
          objectCounts: generation.objectCounts,
          objectTypesWithSuggestions: generation.objectTypesWithSuggestions,
          logCount: generation.logCount,
          traceabilityCount: generation.traceabilityCount,
...
function assertSmokeResult(body, options) {
  ...
  if (!generation) {
    ...
    if (generation.mode !== "real") {
      failures.push("mode is not real");
    }

    if (generation.provider !== options.expectProvider) {
      failures.push(`provider is not ${options.expectProvider}`);
    }
    ...
    if (!(generation.totalObjects > 0)) {
      failures.push("generated object count is empty");
    }

    if (!(generation.objectTypesWithSuggestions > 0)) {
      failures.push("usable model suggestions are empty");
    }

    if (!(generation.logCount > 0)) {
      failures.push("generation log count is empty");
    }

    if (!(generation.traceabilityCount > 0)) {
      failures.push("traceability count is empty");
    }
  }

  if (failures.length > 0) {
    console.error(JSON.stringify({ failures }, null, 2));
    fail("Master Data AI smoke response did not meet the acceptance checks.");
  }
}
...
function printHelp() {
  console.log(`Usage: node scripts/master-data-ai-smoke.mjs [options]
...
The script prints only safe smoke summary fields. It never prints the smoke token, API keys, Vercel protection bypass secret, or generated Master Data text.`);
}
